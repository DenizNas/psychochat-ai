...
def test_crisis_emergency_flow():
    from src.services.database import init_db, SessionLocal, ChatHistory, EmotionEvent
    app.dependency_overrides[get_current_user] = lambda: "test_user_crisis"
    
    # Clean up database records
    init_db()
    db = SessionLocal()
    try:
        db.query(ChatHistory).filter(ChatHistory.user_id == "test_user_crisis").delete()
        db.query(EmotionEvent).filter(EmotionEvent.user_id == "test_user_crisis").delete()
        db.commit()
    finally:
        db.close()
