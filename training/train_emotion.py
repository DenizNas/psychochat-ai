...
# Adjust path so src module can be found
...
def main():
    parser = argparse.ArgumentParser(description="Production Emotion Training Pipeline")
    parser.add_argument("--config", type=str, default="training/configs/emotion_config.json", help="Path to config file")
    args = parser.parse_args()

    logger = setup_logger("train_emotion", "training/logs")
    logger.info("Starting Emotion Training Pipeline...")
    ...
    try:
        ...
        logger.info("Starting Final Test Evaluation on Best Model (if loaded) or Last Model...")
